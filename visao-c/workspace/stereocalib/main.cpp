...
vector<Mat*> LoadImages(string RightFileList, string LeftFileList);
...
int main(int argc, char **argv) {
    ...
    if (!loadCameraParameters()){
        cerr << "Running Calibratation..." << endl;
        StereoCalib(ImageGray, boardSize, 1.0, false, false, false);  
        
        Images = LoadImages("imageListRight.txt", "imageListLeft.txt");
    
        for (int i=0; i < Images.size(); i++){        
            Mat imageGray;
            cvtColor(*Images[i], imageGray, COLOR_BGR2GRAY);  
            ImageGray.push_back(imageGray);
    ...
    cout << "Found Camera parameters, testing disparity..." << endl;
   
    
    frameRight = imread("left_00000.png");
    frameLeft = imread("right_00000.png");
    
    stereoMatch(frameLeft, frameRight);   
...
vector<Mat*> LoadImages(string RightFileList, string LeftFileList){
    ...
    RightFile.open (RightFileList, ios::in);
    LeftFile.open(LeftFileList, ios::in);
    
    if (!RightFile.is_open() || !LeftFile.is_open()) {
        cerr << "Error opening " << RightFileList << " or " <<  LeftFileList << endl;
        exit(EXIT_FAILURE);
    }
    
    while (getline (LeftFile,lineText) ) {      
        ImageName = new string(lineText);        
        LeftImageNames.push_back(ImageName);        
    }
    
    while (getline (RightFile,lineText) ) {      
        ImageName = new string(lineText);        
        RightImageNames.push_back(ImageName);        
    }
    
    if (RightImageNames.size() != LeftImageNames.size()){
        cerr << "Wrong number of files in " << RightFileList << " or " <<  LeftFileList << endl;
        exit(EXIT_FAILURE);
    }
    
    for (int i=0; i < LeftImageNames.size(); i++){
        
        Image = new Mat();        
        *Image = imread(*LeftImageNames[i], IMREAD_COLOR );
        Images.push_back(Image);
        
        Image = new Mat();        
        *Image = imread(*RightImageNames[i], IMREAD_COLOR );
        Images.push_back(Image);
                
    }
    
    cout << "Loaded " << Images.size() << endl;
    
    /* Free string names */
    for (int i=0; i < RightImageNames.size(); i++){
        ImageName = RightImageNames[i];
        delete ImageName;
        
        ImageName = LeftImageNames[i];
        delete ImageName;        
    }
...
void StereoCalib(const vector<Mat>& imagelist, Size boardSize, float squareSize, bool displayCorners, bool useCalibrated, bool showRectified)
{
